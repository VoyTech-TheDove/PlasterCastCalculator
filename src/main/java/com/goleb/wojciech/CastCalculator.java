...
public class CastCalculator {
    ...
    private static final double PRICE_TO_COST_RATIO = 100;

    public BigDecimal calculationOfPrice(List<? extends Figure> shapes) {
        double areaInMM2 = calculateTotalAreaInMM2(shapes);
        double plasterVolume = calculateVolumeInLiters(areaInMM2);
        double plasterCost = calculatePlasterCost(plasterVolume);
        double totalPrice= PRICE_TO_COST_RATIO*plasterCost;
